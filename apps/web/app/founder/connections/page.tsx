...
function ConnectionsContent() {
  ...
  const fetchRequests = async () => {
    try {
      const res = await fetch(`${API_URL}/connection-request/founder`, {
  ...
  const handleReview = async (id: string, decision: 'APPROVE' | 'DECLINE') => {
    ...
    try {
      const res = await fetch(`${API_URL}/connection-request/${id}/review`, {
        method: 'PATCH',
        headers: {
  ...
  if (isLoading) {
    return <div className="min-h-screen flex items-center justify-center text-gray-400 text-sm">Loading connections...</div>;
  }
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto px-4 py-10">
        <h1 className="text-2xl font-bold text-gray-900 mb-1">Connection Requests</h1>
        <p className="text-sm text-gray-500 mb-6">Approving a request shares contact details by email with both parties.</p>
            ...
            <div key={req.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  {req.investor.investmentThesis && (
                    <p className="text-sm text-gray-600 mt-2"><strong>Thesis:</strong> {req.investor.investmentThesis}</p>
                  )}
                  {req.introNote && (
                    <p className="text-sm text-gray-700 mt-2 bg-gray-50 rounded-lg px-3 py-2">&ldquo;{req.introNote}&rdquo;</p>
                  )}
                  {req.investor.linkedinUrl && (
                    <a href={req.investor.linkedinUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-indigo-600 font-medium block mt-2">
                      LinkedIn →
                    </a>
                  )}
